...
func main() {
	...
	for i := range nums {
		for j := range nums[i] {
			...
			if j+3 < len(nums[i]) { //right
				a = nums[i][j] * nums[i][j+1] * nums[i][j+2] * nums[i][j+3]
			}
			if i+3 < len(nums) { //down
				b = nums[i][j] * nums[i+1][j] * nums[i+2][j] * nums[i+3][j]
			}
			if j-3 > 0 { //left
				c = nums[i][j] * nums[i][j-1] * nums[i][j-2] * nums[i][j-3]
			}
			if i-3 > 0 { //up
				d = nums[i][j] * nums[i-1][j] * nums[i-2][j] * nums[i-3][j]
			}
			if j+3 < len(nums[i]) && i+3 < len(nums) { //right down
				e = nums[i][j] * nums[i+1][j+1] * nums[i+2][j+2] * nums[i+3][j+3]
			}
			if j-3 > 0 && i+3 < len(nums) { //left down
				f = nums[i][j] * nums[i+1][j-1] * nums[i+2][j-2] * nums[i+3][j-3]
			}
			if j-3 > 0 && i-3 > 0 { //left up
				g = nums[i][j] * nums[i-1][j-1] * nums[i-2][j-2] * nums[i-3][j-3]
			}
			if j+3 < len(nums[i]) && i-3 > 0 { //right up
				h = nums[i][j] * nums[i-1][j+1] * nums[i-2][j+2] * nums[i-3][j+3]
			}
			max := getMax(a, b, c, d, e, f, g, h)
			if result < max {
				result = max
			}
		}
